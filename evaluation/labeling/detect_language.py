...
def detect_primary_language(row, concepts_lookup: dict[str, str]) -> str:
    ...
    for name in (저자, 제목):
        ...
        for tok in tokens:
            tok = tok.strip()
            if len(tok) >= MIN_SURFACE_LEN and tok in concepts_lookup:
...
def detect_secondary_horizon(
    ...
    dom = _unicode_dominance(raw)
    if dom == "english":
        # Latin dominant — 독일어/프랑스어 키워드 확인
